document.addEventListener('DOMContentLoaded', function () {
  ...
  (function () {
    ...
    function getErrorMessage(field, type) {
      const label = field.closest('.form-group, .field-wrapper, label')
        ? (field.closest('.form-group, .field-wrapper')
            ...
            : null)
        : null;
      const fieldName = label ? label.textContent.trim() : (field.placeholder || field.name || 'This field');
      ...
      return messages[type] || fieldName + ' is invalid.';
    }
    ...
    function clearError(field) {
      ...
      if (wrapper) {
        const existingError = wrapper.querySelector('.form-error-msg');
    ...
    function showError(field, message) {
      ...
      if (wrapper) {
        ...
        errorEl.id = 'error-' + (field.id || field.name || Math.random().toString(36).substr(2, 9));
    ...
    function validateField(field) {
      ...
      if (field.dataset.minlength) {
        ...
        if (!validators.minLength(value, minLen)) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      fetch(action, {
        ...
        .catch(function () {
          const errorContainer = form.querySelector('.form-server-error');
          if (errorContainer) {
  ...
  (function () {
    const ANIMATE_CLASS = 'fade-in-element';
    const VISIBLE_CLASS = 'is-visible';
    const THRESHOLD = 0.15;

    const elements = document.querySelectorAll(
      '.animate-on-scroll, [data-animate], .fade-in, .slide-up, .slide-in'
    );
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "דפוס אותי משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const apiUrl = (window.ZAPPY_API_BASE || 'http://localhost:5001').replace(/\/$/, '');
                ...
                const payload = {
                    websiteId: '91b39182-2585-40e7-9fa1-bea09fb83d9f',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '91b39182-2585-40e7-9fa1-bea09fb83d9f';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '91b39182-2585-40e7-9fa1-bea09fb83d9f';
